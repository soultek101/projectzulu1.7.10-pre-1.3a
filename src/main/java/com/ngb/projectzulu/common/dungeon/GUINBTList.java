...
public class GUINBTList extends GuiScrollingList {
    ...
    protected void elementClicked(int clickedIndex, boolean var2) {
    	 ProjectZuluLog.info("Clicked on Tag of Type %s with name %s", nodeList.get(clickedIndex).getData().toString()
                 //.func_150283_g(nodeList.get(clickedIndex).getData().getId())
         		, nodeList.get(clickedIndex).getTagName());
